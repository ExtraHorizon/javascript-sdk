...
export function createOAuth2HttpClient(
  ...
  httpWithAuth.interceptors.request.use((config: HttpRequestConfig) => ({
    ...config,
    retry: config.retry
      ? config.retry
      : {
  ...
  httpWithAuth.interceptors.response.use(null, (error: HttpResponseError) => {
    const { config } = error;
    const { retry } = config;

    if (error && error.isAxiosError) {
      console.log('current', retry.current);
